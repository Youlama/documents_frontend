...
const DocumentsListPage = ({documents, setDocuments, isMock, setIsMock}:Props) => {
    ...
    const {document_name} = useAppSelector((state) => state.documents)
    ...
    const fetchDocuments = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/documents/?document_name=${document_name.toLowerCase()}`)
