...
import {T_Request, T_RequestsFilters, T_Document, T_RequestCart} from "modules/types.ts";
...
export const fetchCart = createAsyncThunk<T_RequestCart, void, AsyncThunkConfig>(
    "requests/cart",
    async function() {
        const response = await api.cart.cartList() as AxiosResponse<T_RequestCart>
...
export const fetchRequest = createAsyncThunk<T_Request, string, AsyncThunkConfig>(
    ...
    async function(request_id) {
        const response = await api.requests.requestsRead(request_id) as AxiosResponse<T_Request>
        return response.data
    }
)
...
export const removeDocumentFromDraftRequest = createAsyncThunk<T_Document[], string, AsyncThunkConfig>(
    ...
    async function(document_id, thunkAPI) {
        const state = thunkAPI.getState()
        const response = await api.requests.requestsDeleteDocumentDelete(state.requests.request.id, document_id) as AxiosResponse<T_Document[]>
        return response.data
    }
)
...
export const deleteDraftRequest = createAsyncThunk<void, object, AsyncThunkConfig>(
    ...
    async function(_, {getState}) {
        ...
        await api.requests.requestsDeleteDelete(state.requests.request.id)
    }
)
...
export const sendDraftRequest = createAsyncThunk<void, object, AsyncThunkConfig>(
    ...
    async function(_, {getState}) {
        const state = getState()
        await api.requests.requestsUpdateStatusUserUpdate(state.requests.request.id)
    }
)
...
export const updateRequest = createAsyncThunk<void, object, AsyncThunkConfig>(
    ...
    async function(data, {getState}) {
        const state = getState()
        await api.requests.requestsUpdateUpdate(state.requests.request.id, {
            ...data
        })
    }
)
...
export const updateDocumentValue = createAsyncThunk<void, object, AsyncThunkConfig>(
    ...
    async function({document_id, comment},thunkAPI) {
        const state = thunkAPI.getState()
        await api.requests.requestsUpdateDocumentUpdate(state.requests.request.id, document_id, {comment})
    }
)
