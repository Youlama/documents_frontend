...
import {T_RequestsFilters} from "modules/types.ts";
...
const RequestsPage = () => {
    ...
    const filters = useAppSelector<T_RequestsFilters>((state) => state.requests.filters)
    ...
    useEffect(() => {
        const intervalId = setInterval(handleFetchRequests, 2000)
    ...
    const applyFilters = async (e) => {
        ...
        const filters:T_RequestsFilters = {
            status: status,
    ...
    return (
        <Container>
            ...
            {requests.length ? <RequestsTable requests={requests} /> : <h3 className="text-center mt-5">Заявки не найдены</h3>}
        </Container>
    )
};
